...
```{r, include=FALSE}
old.prefix <- getOption("icecream_prefix")
```
...
This option can be used inline in a single `ic()` call as a function parameter.
...
```{r, include=FALSE}
options(icecream_prefix = old.prefix)
```
...
```{r, include=FALSE}
old.ctx <- getOption("icecream_always_include_context")
```
...
If you want to enforce context printing for a single `ic()` call, you can do this using named parameter
to `ic()` function.
...
```{r, include=FALSE}
options(icecream_always_include_context = old.ctx)
```

### `icecream_peeking_function` and `icecream_max_lines`
These two options control how the result of evaluation of an expression is printed. `icecream_peeking_function` indicates the function that summarizes the object. Default value is `ic_autopeek`, which works like `utils::str` for most of the time, but gives more informative output
for `lists`, `data.frames` and their subclasses in a more compact way. `icecream_max_lines` 
determines maximum number of lines that the peek of an object occupies. By default (value of `NA`) code selects the predefined number of lines specified for a number of predefined *peeking functions*. If a numeric value is provided, it overrides the default behavior (see package documentation for details).

For more complex data you may want to use e.g. `head` function. Predefined value of `max_lines` for `head` is 5.

```{r, include=FALSE}
old.fun <- getOption("icecream_peeking_function")
old.lines <- getOption("icecream_max_lines")
```
...
Those options can be used in an inline call as well. See that on the example of custom peeking function.
...
```{r, include=FALSE}
options(icecream_peeking_function = old.fun,
        icecream_max_lines = old.lines)
```

Note that if `icecream_max_lines` is greater than 1 and summary of an object is longer than 1, the 
...
* Implement `ic.output_function`. At the moment it uses `cli::cli_alert_info()`
